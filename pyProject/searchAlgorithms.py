...
def iterative_depth(graph, start, goal, step_by_step=False):
    ...
    for depth in range(len(graph)):
        ...
        path = limited_depth_search(graph, start, goal, depth, step_by_step=step_by_step)
        if path:
            if step_by_step:
    ...
    while current_node != goal:
        ...
        for next_node in destinations:
            ...
            if next_node not in shortest_path:
                ...
                if step_by_step:
                    print(f"Agregando o actualizando nodo: {next_node.title()} con peso: {weight}")
            else:
                current_shortest_weight = shortest_path[next_node][1]
                if current_shortest_weight > weight:
                    shortest_path[next_node] = (current_node, weight)
                    if step_by_step:
                        print(f"Actualizando el camino más corto para: {next_node.title()} con nuevo peso: {weight}")
...
def depth_first_search(graph, start, goal, step_by_step=False, path=None):
    ...
    for neighbor in graph.neighbors(start):
        if neighbor not in path:
            if step_by_step:
                print("Explorando desde:", start.title(), "a vecino:", neighbor.title())
            new_path = depth_first_search(graph, neighbor, goal, step_by_step, path + [neighbor])
            if new_path:
                return new_path
